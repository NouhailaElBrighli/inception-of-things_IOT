...
def wait_for_anajmi_app_pods():
    """Wait for the 'anajmi-app' pods to be ready and report elapsed time."""
    colpr(
        "y",
        "WAIT until anajmi-app pods are ready before starting... (This can take up to 6 minutes)",
    )
    start = time.time()
    wait_proc = run(
    ...
    if wait_proc.returncode != 0:
        notify("App Error", "anajmi-app pods creation timeout")
        colpr("r", "An error occurred. The creation of anajmi-app pods timed out.")
        sys.exit(1)
    elapsed = elapsed_time(start)
    colpr("y", f"{elapsed} elapsed since waiting for anajmi-app pods creation.")


def update_deployment_image():
    """Clone the repo, update deployment.yaml, commit and push changes."""
    # Port-forward anajmi-app-service on port 8888 for testing
    run("kubectl config set-context --current --namespace=dev")
    kill_portforward("kubectl port-forward svc/anajmi-app-service")
    port_forward("anajmi-app-service", "dev", 8888, 8888)
    time.sleep(5)

    # Retrieve the current image version using regex matching.
    res = run(
        "kubectl describe deployments anajmi-app-deployment | grep 'Image'",
        capture_output=True,
    )
    ...
    run("kubectl describe deployments anajmi-app-deployment | grep 'Image'")
    ...
    run(
        "git clone 'http://gitlab.localhost:8081/root/anajmi_IOT.git' tmp",
...
def wait_for_sync(newImageVersion):
    ...
    cmd = f'kubectl wait deployment anajmi-app-deployment --for=jsonpath="{{.spec.template.spec.containers[*].image}}"="wil42/playground:v{newImageVersion}" --timeout=600s'
    wait_sync = run(cmd)
...
def test_app_with_curl():
    ...
    time.sleep(5)
    port_forward("anajmi-app-service", "dev", openPort, 8888)
    time.sleep(10)
    ...
    while curl_proc.returncode != 0:
        ...
        result_pf = run(
            f"ps aux | grep -v 'grep' | grep 'kubectl port-forward svc/anajmi-app-service -n dev {openPort}'",
            capture_output=True,
        )
